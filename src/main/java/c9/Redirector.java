 ...
 * A Redirector
 *
 * Redirection is another simple but useful application for a special-purpose HTTP server. In this section, you develop
 * a server that redirects users from one website to another—for example, from cnet.com to www.cnet.com. Example 9-11
 * reads a URL and a port number from the command line, opens a server socket on the port, and redirects all requests
 * that it receives to the site indicated by the new URL using a 302 FOUND code. In this example, I chose to use a new
 * thread rather than a thread pool for each connection. This is perhaps a little simpler to code and understand but
